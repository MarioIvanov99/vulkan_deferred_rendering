...
namespace
{
	std::vector<VkSurfaceFormatKHR> get_surface_formats(VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface)
	{
		...
		if (auto const res = vkGetPhysicalDeviceSurfaceFormatsKHR(aPhysicalDev, aSurface, &surfCount, nullptr); VK_SUCCESS != res)
		{
			std::fprintf(stderr, "Error: unable to enumerate format\n");
			std::fprintf(stderr, "vkGetPhysicalDeviceSurfaceFormatsKHR() returned %s\n", lut::to_string(res).c_str());
			return {};
		}

		std::vector<VkSurfaceFormatKHR> surfaces(surfCount);
		if (auto const res = vkGetPhysicalDeviceSurfaceFormatsKHR(aPhysicalDev, aSurface, &surfCount, surfaces.data()); VK_SUCCESS != res)
		{
			std::fprintf(stderr, "Error: unable to get format\n");
			std::fprintf(stderr, "vkGetPhysicalDeviceSurfaceFormatsKHR() returned %s\n", lut::to_string(res).c_str());
			return {};
	...
	std::unordered_set<VkPresentModeKHR> get_present_modes(VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface)
	{
		...
		{
			std::fprintf(stderr, "Error: unable to enumerate modes\n");
		...
		{
			std::fprintf(stderr, "Error: unable to get modes\n");
			std::fprintf(stderr, "vkGetPhysicalDeviceSurfacePresentModesKHR() returned %s\n", lut::to_string(res).c_str());
			return {};

		}

		std::unordered_set<VkPresentModeKHR> modesReturn(modes.begin(), modes.end());
		return modesReturn;
	}
	...
	std::tuple<VkSwapchainKHR, VkFormat, VkExtent2D> create_swapchain(VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface, VkDevice aDevice, GLFWwindow* aWindow, std::vector<std::uint32_t> const& aQueueFamilyIndices, VkSwapchainKHR aOldSwapchain)
	{
		...
		// Prefer FIFO RELAXED if itÅfs available. 
		if (modes.count(VK_PRESENT_MODE_FIFO_RELAXED_KHR))
			presentMode = VK_PRESENT_MODE_FIFO_RELAXED_KHR;

		// Pick an image count 1
		...
		else
		{
			...
			// require explicit ownership transfers, which weÅfre avoiding for 
	...
	void get_swapchain_images(VkDevice aDevice, VkSwapchainKHR aSwapchain, std::vector<VkImage>& aImages)
	{
		...
		if (auto const res = vkGetSwapchainImagesKHR(aDevice, aSwapchain, &imageNum, nullptr); VK_SUCCESS != res)
		{
			std::fprintf(stderr, "Error: unable to enumerate images\n");
			std::fprintf(stderr, "vkGetSwapchainImagesKHR() returned %s\n", lut::to_string(res).c_str());

		}

		aImages.resize(imageNum);

		if (auto const res = vkGetSwapchainImagesKHR(aDevice, aSwapchain, &imageNum, aImages.data()); VK_SUCCESS != res)
		{
			std::fprintf(stderr, "Error: unable to get images\n");
			std::fprintf(stderr, "vkGetSwapchainImagesKHR() returned %s\n", lut::to_string(res).c_str());
...
namespace
{
	float score_device(VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface)
	{
		...
		{
			std::fprintf(stderr, "Info: Discarding device Åf%sÅf: extension %s missing\n", props.deviceName, VK_KHR_SWAPCHAIN_EXTENSION_NAME);
			return -1.f;
		}
		...
		{
			std::fprintf(stderr, "Info: Discarding device Åf%sÅf: canÅft present to surface\n", props.deviceName);
			return -1.f;
		}
		...
		{
			std::fprintf(stderr, "Info: Discarding device Åf%sÅf: no graphics queue family\n", props.deviceName);
			return -1.f;
		}
