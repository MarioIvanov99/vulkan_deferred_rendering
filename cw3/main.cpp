...
#		define SHADERDIR_ "assets/cw3/shaders/"
		constexpr char const* kTexVertShaderPath = SHADERDIR_ "triangle.vert.spv";
		constexpr char const* kTexFragShaderPath = SHADERDIR_ "triangle.frag.spv";
#		undef SHADERDIR_

		//Not actually used in this coursework
//#		define SHADERDIR_ "assets/cw3/shaders/"
...
#		undef SCENEDIR_
	...
	//There is a second of each of these, for the second render pass
	lut::DescriptorSetLayout create_scene_descriptor_layout(lut::VulkanWindow const&);
	lut::DescriptorSetLayout create_second_scene_descriptor_layout(lut::VulkanWindow const&);

	lut::PipelineLayout create_pipeline_layout(lut::VulkanContext const&, VkDescriptorSetLayout aSceneLayout);
	lut::PipelineLayout create_second_pipeline_layout(lut::VulkanContext const&, VkDescriptorSetLayout aSceneLayout, VkDescriptorSetLayout aTexLayout);
	...
	std::tuple<lut::Image, lut::ImageView> create_image_buffer(lut::VulkanWindow const&, lut::Allocator const&, VkFormat);
	...
	lut::Framebuffer create_first_framebuffer( //Many attachments
		lut::VulkanWindow const&,
		VkRenderPass,
		VkImageView aColorView,
		VkImageView aNormalView,
		VkImageView aNotherView,
		VkImageView aPosView,
	...
	void record_commands(
		...
		VkDescriptorSet aSceneDescriptors2,
		VkImage, //For synchronization
		VkImage,
		VkImage,
		VkImage
	);
...
int main() try
{
	...
	lut::RenderPass renderPass2 = create_second_render_pass(window);

	lut::DescriptorSetLayout sceneLayout = create_scene_descriptor_layout(window);
	lut::DescriptorSetLayout secondSceneLayout = create_second_scene_descriptor_layout(window);

	lut::PipelineLayout pipeLayout = create_pipeline_layout(window, sceneLayout.handle);
	lut::PipelineLayout pipeLayout2 = create_second_pipeline_layout(window, sceneLayout.handle, secondSceneLayout.handle);

	lut::Pipeline pipe = create_pipeline(window, renderPass.handle, pipeLayout.handle);
	lut::Pipeline pipe2 = create_second_pipeline(window, renderPass2.handle, pipeLayout2.handle);

	//For each attachment:
	//Every color attachment uses the same format.
	auto [depthBuffer, depthBufferView] = create_depth_buffer(window, allocator);
	auto [imageBuffer, imageBufferView] = create_image_buffer(window, allocator, VK_FORMAT_R32G32B32A32_SFLOAT);
	auto [normalBuffer, normalBufferView] = create_image_buffer(window, allocator, VK_FORMAT_A2B10G10R10_UNORM_PACK32);
	auto [otherBuffer, otherBufferView] = create_image_buffer(window, allocator, VK_FORMAT_R32G32B32A32_SFLOAT);
	auto [posBuffer, posBufferView] = create_image_buffer(window, allocator, VK_FORMAT_R32G32B32A32_SFLOAT);

	//Create_first_framebuffer is not a void because it only needs one, unlike the swapchain
	lut::Framebuffer framebuffers = create_first_framebuffer(window, renderPass.handle,
		imageBufferView.handle, normalBufferView.handle, otherBufferView.handle, posBufferView.handle, depthBufferView.handle);
	std::vector<lut::Framebuffer> framebuffers2; 

	create_swapchain_framebuffers(window, renderPass2.handle, framebuffers2);
	...
	for (std::size_t i = 0; i < framebuffers2.size(); ++i)
	{
	...
	VkDescriptorSet texDescriptor = lut::alloc_desc_set(window, dpool.handle, secondSceneLayout.handle);
	{
		VkWriteDescriptorSet desc[4]{};

		VkDescriptorImageInfo textureInfo{};
		textureInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		textureInfo.imageView = imageBufferView.handle;
		...
		VkDescriptorImageInfo textureInfoN{};
		textureInfoN.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		textureInfoN.imageView = normalBufferView.handle;
		textureInfoN.sampler = defaultSampler.handle;

		desc[1].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		desc[1].dstSet = texDescriptor;
		desc[1].dstBinding = 1;
		desc[1].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
		desc[1].descriptorCount = 1;
		desc[1].pImageInfo = &textureInfoN;

		VkDescriptorImageInfo textureInfoO{};
		textureInfoO.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		textureInfoO.imageView = otherBufferView.handle;
		textureInfoO.sampler = defaultSampler.handle;

		desc[2].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		desc[2].dstSet = texDescriptor;
		desc[2].dstBinding = 2;
		desc[2].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
		desc[2].descriptorCount = 1;
		desc[2].pImageInfo = &textureInfoO;

		VkDescriptorImageInfo textureInfoP{};
		textureInfoP.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		textureInfoP.imageView = posBufferView.handle;
		textureInfoP.sampler = defaultSampler.handle;

		desc[3].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		desc[3].dstSet = texDescriptor;
		desc[3].dstBinding = 3;
		desc[3].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
		desc[3].descriptorCount = 1;
		desc[3].pImageInfo = &textureInfoP;
	...
	while (!glfwWindowShouldClose(window.window))
	{
		...
		if (recreateSwapchain)
		{
			...
			if (changes.changedFormat) {
				...
				renderPass2 = create_second_render_pass(window);
			}

			if (changes.changedSize) {
				std::tie(depthBuffer, depthBufferView) = create_depth_buffer(window, allocator);
				std::tie(imageBuffer, imageBufferView) = create_image_buffer(window, allocator, VK_FORMAT_R32G32B32A32_SFLOAT);
				std::tie(normalBuffer, normalBufferView) = create_image_buffer(window, allocator, VK_FORMAT_A2B10G10R10_UNORM_PACK32);
				std::tie(otherBuffer, otherBufferView) = create_image_buffer(window, allocator, VK_FORMAT_R32G32B32A32_SFLOAT);
				std::tie(posBuffer, posBufferView) = create_image_buffer(window, allocator, VK_FORMAT_R32G32B32A32_SFLOAT);
			}

			framebuffers2.clear();
			framebuffers = create_first_framebuffer(window, renderPass.handle, imageBufferView.handle, normalBufferView.handle, otherBufferView.handle, posBufferView.handle, depthBufferView.handle);
			create_swapchain_framebuffers(window, renderPass2.handle, framebuffers2);

			if (changes.changedSize) {
				pipe = create_pipeline(window, renderPass.handle, pipeLayout.handle);
				pipe2 = create_second_pipeline(window, renderPass2.handle, pipeLayout2.handle);
			}
		...
		assert(std::size_t(imageIndex) < framebuffers2.size());

		record_commands(cbuffers[imageIndex], renderPass.handle, renderPass2.handle, framebuffers.handle, framebuffers2[imageIndex].handle,
			pipe.handle, pipe2.handle, window.swapchainExtent,
			aPositionBuffer, ANormalBuffer, aDiffuseBuffer, aSpecularBuffer, aEmissiveBuffer, aAlbedoBuffer, aShinyBuffer, aVertexCount,
			sceneUBO.buffer, sceneUniforms, lightUBO.buffer, lightUniforms,
			pipeLayout.handle, pipeLayout2.handle, sceneDescriptors, texDescriptor,
			imageBuffer.image, normalBuffer.image, otherBuffer.image, posBuffer.image);
...
namespace
{
	void update_scene_uniforms(glsl::SceneUniform& aSceneUniforms, std::uint32_t aFramebufferWidth, std::uint32_t aFramebufferHeight)
	{
		...
		aSceneUniforms.cameraPos = glm::vec4(cfg::pos, 1.0);

		aSceneUniforms.windowSize.x = (float)aFramebufferWidth; //Added for attempts to reconstruct world coordinates
		aSceneUniforms.windowSize.y = (float)aFramebufferHeight;
	...
	void update_light_uniforms(glsl::LightUniform& aLightUniforms, float rotation, float num_lights) {
		aLightUniforms.light_col[0] = glm::vec4(1.0f, 1.0f, 0.8f, 1.0);
...
namespace
{
	lut::RenderPass create_render_pass(lut::VulkanWindow const& aWindow)
	{
		...
		subpassAttachments[1].attachment = 1; // this refers to attachments[1] 
		subpassAttachments[1].layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;

		subpassAttachments[2].attachment = 2; // this refers to attachments[2] 
		subpassAttachments[2].layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;

		subpassAttachments[3].attachment = 3; // this refers to attachments[3] 
		subpassAttachments[3].layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;

		VkAttachmentReference depthAttachment{};
		depthAttachment.attachment = 4; // this refers to attachments[4] 
	...
	lut::PipelineLayout create_second_pipeline_layout(lut::VulkanContext const& aContext, VkDescriptorSetLayout aSceneLayout, VkDescriptorSetLayout aTexLayout)
	{
		VkDescriptorSetLayout layouts[] = {
			// Order must match the set = N in the shaders 
			aSceneLayout,
	...
	{
		//Unchanged, except the addition of new attachments
	...
	lut::Framebuffer create_first_framebuffer(lut::VulkanWindow const& aWindow, VkRenderPass aRenderPass, VkImageView aColorView, VkImageView aNormalView, VkImageView aNotherView, VkImageView aPosView, VkImageView aDepthView)
	{
			...
			if (auto const res = vkCreateFramebuffer(aWindow.device, &fbInfo, nullptr, &fb); VK_SUCCESS != res)
			{
				throw lut::Error("Unable to create framebuffer for swap chain image %zu\n" "vkCreateFramebuffer() returned %s", 0, lut::to_string(res).c_str());
	...
	lut::DescriptorSetLayout create_second_scene_descriptor_layout(lut::VulkanWindow const& aWindow) {
		...
		bindings[2].binding = 2; // number must match the index of the corresponding 
		bindings[2].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
		bindings[2].descriptorCount = 1;
		bindings[2].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;

		bindings[3].binding = 3; // number must match the index of the corresponding 
	...
	void record_commands(VkCommandBuffer aCmdBuff, VkRenderPass aRenderPass, VkRenderPass aRenderPass2, VkFramebuffer aFramebuffer, VkFramebuffer aFramebuffer2,
		VkPipeline aGraphicsPipe, VkPipeline aGraphicsPipe2, VkExtent2D const& aImageExtent,
		...
		VkBuffer aSceneUBO, glsl::SceneUniform const& aSceneUniform, VkBuffer aLightUBO, glsl::LightUniform const& aLightUniform, VkPipelineLayout aGraphicsLayout,
		VkPipelineLayout aGraphicsLayout2, VkDescriptorSet aSceneDescriptors, VkDescriptorSet aSceneDescriptors2,
		VkImage image, VkImage normalImage, VkImage otherImage, VkImage posImage)
	{
		...
		// Begin render pass 
		{
			VkClearValue clearValues[5]{}; //Need extra clear values for each attachment
		...
		//Barriers for sync
		lut::image_barrier(aCmdBuff, image,
		...
		lut::image_barrier(aCmdBuff, normalImage,
			VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,
			VK_ACCESS_SHADER_READ_BIT,
			VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
			VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
		...
		lut::image_barrier(aCmdBuff, otherImage,
			VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,
			VK_ACCESS_SHADER_READ_BIT,
			VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
			VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
		...
		lut::image_barrier(aCmdBuff, posImage,
			VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,
			VK_ACCESS_SHADER_READ_BIT,
			VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
			VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
		...
		//No vertex buffers
		{
			...
			VkRenderPassBeginInfo passInfo{};
			passInfo.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
			passInfo.renderPass = aRenderPass2;
			passInfo.framebuffer = aFramebuffer2;
			passInfo.renderArea.offset = VkOffset2D{ 0, 0 };
			passInfo.renderArea.extent = aImageExtent;
			...
			// Begin drawing with our graphics pipeline 
			vkCmdBindPipeline(aCmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, aGraphicsPipe2);
			vkCmdBindDescriptorSets(aCmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, aGraphicsLayout, 0, 1, &aSceneDescriptors, 0, nullptr);
			vkCmdBindDescriptorSets(aCmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, aGraphicsLayout2, 1, 1, &aSceneDescriptors2, 0, nullptr);
...
namespace
{
	...
	std::tuple<lut::Image, lut::ImageView> create_image_buffer(lut::VulkanWindow const& aWindow, lut::Allocator const& aAllocator, VkFormat vformat)
	{
		VkImageCreateInfo imageInfo{};
		imageInfo.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
		imageInfo.imageType = VK_IMAGE_TYPE_2D;
